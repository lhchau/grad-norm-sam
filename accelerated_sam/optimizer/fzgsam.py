    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                vertical_old_g = self.state[p]['old_g'] - cosine * self.old_grad_norm * p.grad.data / (self.new_grad_norm + self.perturb_eps)
                d_norm_d_p = (d_p.sub(vertical_old_g)).div(group['rho'])
                
                if weight_decay != 0:
                    d_p.add_(p.data, alpha=weight_decay)
                    
                d_p.add_(d_norm_d_p, alpha=self.alpha)
